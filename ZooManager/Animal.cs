...
namespace ZooManager
{
    public class Animal: Occupant
    {
        ...
        // Improve Feature: (g) Wirte a new method Move that use 3 pieces of information: an Animal, a Direction, and a Distance. The specified animal will move in the specified direction 1 square at a time, but will only into an empty square. If the next square in that direction is not empty, the animal will not move.
        /// <summary>
        ...
        protected virtual int Move(Animal runner, Direction hunterDirecton, int Distance)
        {
            ...
            bool canMove = false; // Add a bool value to remember whether the animal can move
            ...
            for(int i = 0; i < 3; i++)
            {
                ...
                {
                    // use for loop to make the animal move the distance given by Distance.
                    for (int d = 0; d < Distance; d++)
                    {
                        if (y - d > 0 && x >= 0 && x <= Game.numCellsX - 1 && Game.animalZones[y - (d + 1)][x].occupant == null) // The animal is not out of range in the x-direction and the coordinate y - d remains within the range in the y-direction, while the zone on (y - d, x) is empty.
                        {
                ...
                else if (moveDirection[i] == Direction.down) // direction is down
                {
                    for (int d = 0; d < Distance; d++)
                    {
                        if (y + d < Game.numCellsY - 1 && x >= 0 && x <= Game.numCellsX - 1 && Game.animalZones[y + (d + 1)][x].occupant == null) // The animal is not out of range in the x-direction and the coordinate y + d remains within the range in the y-direction, while the zone on (y + d, x) is empty.
                        {
                            Game.animalZones[y + (d + 1)][x].occupant = runner;
                            Game.animalZones[y + d][x].occupant = null;
                            distance++;
                ...
                else if (moveDirection[i] == Direction.left) // direction is left
                {
                    for (int d = 0; d < Distance; d++)
                    {
                        if (x - d > 0 && y >= 0 && y <= Game.numCellsY -1 && Game.animalZones[y][x - (d + 1)].occupant == null) // The animal is not out of range in the y-direction and the coordinate x - d remains within the range in the x-direction, while the zone on (y, x - d) is empty.
                        {
                ...
                else if (moveDirection[i] == Direction.right)
                {
                    for (int d = 0; d < Distance; d++)
                    {
                        if (x + d < Game.numCellsX - 1 && y >= 0 && y <= Game.numCellsY - 1 && Game.animalZones[y][x + (d + 1)].occupant == null) // The animal is not out of range in the y-direction and the coordinate x + d remains within the range in the x-direction, while the zone on (y, x + d) is empty.
                        {
                            Game.animalZones[y][x + (d + 1)].occupant = runner;
                            Game.animalZones[y][x + d].occupant = null;
                            distance++;
